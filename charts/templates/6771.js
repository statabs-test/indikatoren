		...
		chart: {
			events: {
	            load: function (e) {
					...
					fn.addLegendTitle(chart,  'Bevölkerungssaldo', lp.blockChoropleth.title.x, lp.blockChoropleth.title.y[1]);	
					fn.addLegendTitle(chart, choroplethSeries.name.replace("er ", "er<br/>"), lp.blockSymbol.title.x, lp.blockChoropleth.title.y[1]);
	                	                // fn.addLegendTitle(chart, 'Wanderung/Umzug', 500, 280); //Reihenfolge bei 6022.js falsch xy und dann Titel...
	                fn.addLegendColumnChart(chart, lp.blockSymbol.x[1], lp.blockSymbol.y4S[0]+15,  legendColumnValues.map(function(x) { return x * 0.8; }), color, 'columnLegendHideOnZoom'); //Hemdsärmlige Lösung MB
	                fn.addLegendText(chart, lp.blockSymbol.numbers.x + 10, lp.blockSymbol.numbers.y4S[0],  Highcharts.numberFormat(3.0, 1,","," "), color(3, 0), 'columnLegendHideOnZoom');
	                fn.addLegendText(chart, lp.blockSymbol.numbers.x + 35, lp.blockSymbol.numbers.y4S[0],  Highcharts.numberFormat(1.5, 1,","," "), color(1.5, 1), 'columnLegendHideOnZoom');

					fn.addLegendSquare(chart,      lp.blockSymbol.x[1], lp.blockSymbol.y4S[0]+21+5,  15, color(1, 0));
					fn.addLegendText(chart,        lp.blockSymbol.numbers.x, lp.blockSymbol.numbers.y4S[0]+21+5,  'Wanderung positiv');
					fn.addLegendSquare(chart,      lp.blockSymbol.x[1], lp.blockSymbol.y4S[1]+14+5,  15, color(-1, 0));
					fn.addLegendText(chart,        lp.blockSymbol.numbers.x, lp.blockSymbol.numbers.y4S[1]+14+5,  'Wanderung negativ');
					
					fn.addLegendSquare(chart,      lp.blockSymbol.x[1], lp.blockSymbol.y4S[2]+7,  15, color(1, 1));
					fn.addLegendText(chart,        lp.blockSymbol.numbers.x, lp.blockSymbol.numbers.y4S[2]+7,  'Umzug positiv');
					fn.addLegendSquare(chart,      lp.blockSymbol.x[1], lp.blockSymbol.y4S[3],  15, color(-1, 1));
					fn.addLegendText(chart,        lp.blockSymbol.numbers.x, lp.blockSymbol.numbers.y4S[3], 'Umzug negativ');
