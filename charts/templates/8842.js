...
//console.log(pieSizeSeries.data);
	/*	
pieSizeSeries.data.forEach (function(item){
	fn.addLegendLabel(chart, item.options["hc-key"].value, 540, 270, undefined, false, 'right');
	}
);*/
//console.log(choroplethSeries);
for (let i=0; i<pieSizeSeries.data.length; i++) {
//	console.log(pieSizeSeries.data[i].options["hc-key"]);
	fn.addLegendLabel(chart, choroplethSeries.data[i].Wohnviertel_Id +': '+ pieSizeSeries.data[i].options["hc-key"], 800, 30+i*20, undefined, false, 'right');
