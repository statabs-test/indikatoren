...
(function () {
		...
		chart: {
			events: {
				load: function (e) {
					...
					//legend positioning
					var tr = 2, //number of title rows
						lht = 15, //line-height title
						lhc = 25, //line-height circles
						col_pos = '#FABD24', //color for positive circles
						col_neg = '#FFDA80', //color for negative circles
						lx = this.legend.options.x, //230, from chart.legend, see above
						ly = this.legend.options.y, //250, from chart.legend, see above
					...
					var lp = { //legendPosition 
						blockChoropleth: {
							x: lx + 15, // background x
							y: ly - (tr * lht), //background y
							ty: [ly - lht, ly - 2 * lht, ly - 3 * lht], //title y
							tx: lx + 15, //title x
						},
						blockSymbol: {
							x: lx + sox, //background x
					...
					fn.addLegendSquare(chart, lp.blockSymbol.cx, lp.blockSymbol.cy[0], 10, col_pos);
					//fn.addLegendSquare(chart, 565, 240, 10, col_pos);
					fn.addLegendSquare(chart, 565, 256, 10, col_neg);
					fn.addLegendLabel(chart, 'Zunahme', 580, 236);
					fn.addLegendLabel(chart, 'Abnahme', 580, 252);

	//wenn kontinuierliche Bubbles, d.h. nur zwei Referenzgrössen in Legende (und zu-/abnahme-Farben)
	...
	//noch anpassen
					...
					fn.addLegendCircle(chart, lp.blockSymbol.cx, lp.blockSymbol.cy[0], 0.5*fn.pieSize(minValueInLegend, extremeValues.maxAbsNumber, maxPieDiameter), 'grey');
