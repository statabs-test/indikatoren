			...
			drawPies: function (chart, pieSizeSeries, choroplethSeries, pieSeriesConfig, pieSizeCatConfig, color) {
				...
				Highcharts.each(pieSizeSeries.points, function (data) {
					...
					var currentPieSeries = function (config) {
						...
						var labeltxt = correspondingMapSeriesItem.Wohnviertel_Id.toString();
						//var labeltxt = correspondingMapSeriesItem["Wohnviertel Id"].toString();
						/*var labeltxt = correspondingMapSeriesItem.properties.LIBGEO
						...
						var mapPieConfig = {
							...
							tooltip: {
								...
								pointFormatter: function () {
									return labeltxt + ': <b>' + Highcharts.numberFormat((this.v), 3) + '</b><br/>';
								}
							},
							center: {
							...
							dataLabels: { //show dataLabels if set in chart-JS, e.g. for Wohnviertel-Nummerierung (BL, 17.02.2020)
								enabled: (Highcharts.charts[1] !== undefined ? Highcharts.charts[1].userOptions.plotOptions.pie.dataLabels.enabled : false) , //gets setting from chart-js
								useHTML: true,
								zIndex: 0,
								distance: 0,
								padding: 2,
								x: -13 - labeltxt.length / 2 * 4, //move to the left, so it's centered (4 = appr. letter-width in px)
								y: 1,
								borderWidth: 0,
								style: {
									textOutline: '0px',
